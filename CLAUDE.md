...
pnpm --filter @online-rummy/client test
...
- `protocol.ts` — `C2S` and `S2C` discriminated unions (field `t` is the tag). `PileSlice` for 500 Rummy pile-dive data. `EventKind` includes per-action kinds (`drew`/`melded`/`laidOff`/`discarded`/`knocked`/`passedUpcard`) emitted by the server before each action's state broadcast; `DrewEventData = { from: 'stock' | 'discard' | 'pile' }` is the `drew` payload.
- `meld.ts` — `validateMeld(cards, opts)` (`aceHigh`, `roundTheCorner`, `aceEitherEnd`), `cardPoints`, `MeldOptions`. Single runtime source for client + server; replaces the old `engine/meld.ts` and the three client-side mirrors deleted in the Phase 6 refactor. House-rule options: `MeldOptions.setsRequireDistinctSuits?`; `roundTheCorner: true` validates wrap runs (K-A-2 via circular mod-13 check); `cardPoints(card, aceValue, { low5Scoring? })`; `score500MeldCard(card, all, Score500Options)` (`acesAlways15` overrides the A-2-3 ace to 15, precedence over `low5Scoring` per rules.md A.4.2).
- `houseRules.ts` — NS-8 house-rule registry: `HouseRuleId`/`HouseRuleValue`/`HouseRules`/`HouseRuleDef` (+`supported` gate — false ⇒ engine does not honor ⇒ UI hides it), `HOUSE_RULE_DEFS` (basic 5 defs, rum500 6 incl. `jokers` at `supported:false`, gin `[]`), `canonicalHouseRules`, `isDeviation`, `supportedDefs`. Exactly 10 defs are `supported:true` (each engine-honored under golden tests).
...
| File | Purpose |
| --- | --- |
| `engine/types.ts` | `GameState` (discriminated union by `variant` with `variantState` pocket: `BasicState`, `Rum500State`, `GinState`), `GamePlayer`, `VariantEngine` interface (action handlers + lifecycle hooks), `ScoreSheet`, `WonHandData`. Base state carries `houseRules: HouseRules` (stamped at createGame; absent key = canonical). `BasicState = { meldedThisTurn?: boolean }` (optional by design — required would break util.test.ts's empty-pocket call). |
...
| `engine/variants/basic.ts` | `basicVariant: VariantEngine`, `createBasicGame`, `applyDraw`, `applyMeld`, `applyLayoff`, `applyDiscard`. House rules: `meldOptsBasic` (roundTheCorner ⇒ aceEitherEnd at the opts site; config never normalized), unmelded ace = 15 under either ace flag (scoreHand + handDeadwood), `ERR_MAX_ONE_MELD` (`meldedThisTurn` pocket, reset in local advanceTurn wrapper), `ERR_LAYOFF_REQUIRES_MELD`, goingRummyFlat10 ⇒ `earned + 10`. |
| `engine/variants/rum500.ts` | `rum500Variant: VariantEngine`, `createRum500Game`, `applyDraw`, `applyDrawFromPile`, `applyMeld`, `applyLayoff`, `applyDiscard`, plus `r500(state)` narrowing helper. Also owns `runAceDirection`, `score500MeldCard`. House rules: `dealRum500` (2P deals 10 under deal10For2P), unifiedObligation ⇒ top-card draws set `drewFromDiscardId` AND `mustMeldCardId` behind the `ERR_NO_LEGAL_DIVE` preflight, `meldOpts500` (setsRequireDistinctSuits), `score500Opts` (acesAlways15/low5Scoring in meld credit + hand counting). |
...
| `src/store.ts` | Zustand store — all app state. `handleMessage(S2C)` is the single entry point for server messages. `cardCache` holds Card objects by id so melded cards (removed from hand) can still be rendered. Gin staging: `knockMelds`, `ginDefenderMelds`, `ginLayoffs` accumulate client-side declarations before submission; each has add/remove-by-index/clear actions. All three cleared on `draw` phase and `gameStarted`. `meldHighlights` + `meldHighlightOwnerId` track cards just placed (melded or laid off) in basic/rum500 mid-turn play and persist them through the **next** player's pre-draw window — cleared the moment that player draws, or whenever the hand starts/ends, or in Gin (which has no mid-turn melding). `connStatus` (set via `setConnStatus`, which also mirrors the legacy `connected` boolean for Home's form gating) holds the socket lifecycle from `ws.onStatus`. `opponentConn` holds an opponent who dropped mid-game within the server grace window — set on `playerDisconnected`, cleared on `playerReconnected`/`forfeit`/`gameOver`/leave. `playerLastSeen` tracks each player's last-heard time (refreshed by keepalive/event/chat and by every `state` broadcast for all non-self players); `checkDisconnects` (run every 30s from App) flags a non-forfeited player silent for >5min via `disconnectWarning`, cleared on the player's `forfeit`/`gameOver` or by dismiss — this is a slow backstop for the server's faster 60s grace path. `leaveGame` sends `{ t: 'leave' }` and resets to the start page. `sessionStorage` persists `sessionId`, `roomCode`, **and `playerName`** (the last is restored on first load so the create/join form re-populates across sessions). NS-8: `houseRules` field (lobby + publicState) + `setHouseRules(hr)` action + ERROR_MESSAGES for the new house-rule codes. **Sound hook:** the first thing `handleMessage` does (before the switch mutates state) is call `soundForMessage(msg, ctx)` with the genuinely-previous state and `playSound` the result. |
| `src/audio/soundMap.ts` | Pure S2C → `SoundId \| null` decision table (`soundForMessage`), deliberately free of asset imports so node-env tests can load it. Owns the your-turn diff guards (prev-null / ended-phase / turn-transition), wonHand precedence (gin > undercut > go-out > hand-over), own-chat silence, in-game-only error buzz (store-swallowed stale-session codes excluded), lobby-growth join ding. |
| `src/audio/sounds.ts` | Web Audio playback: one lazily-created `AudioContext({ latencyHint: 'interactive' })` feeding a master `GainNode` (`gain.value = muted ? 0 : volume * volume`, a perceptual curve) that every `AudioBufferSourceNode` connects through — so a mute/volume change reaches sounds already in flight, unlike the old per-clone `.volume`. `initAudio()` (idempotent, node-safe) fetches + `decodeAudioData`s all 21 wavs into a `Map<SoundId, AudioBuffer>` and resumes the autoplay-suspended context on the first `pointerdown`/`keydown`. `playSound(id)` submits to `holdPolicy.ts`'s hold gate, then plays via a fresh `AudioBufferSourceNode` per call (per-id concurrency cap 2, decremented in `onended` — leak-proof, no rejected-`play()` guard needed), never throws. Each source also routes through one of two per-category `GainNode` buses feeding the master gain — `action` (draw-stock/draw-discard/pile-dive/meld/layoff/discard/deal) vs `alert` (everything else), chosen by the exhaustive `gainBus()` switch (same pattern as `holdPolicy.ts`'s `cueFamily`) — and `action`-bus cues get a small per-play `playbackRate` jitter (`PLAYBACK_JITTER_PCT = 0.04`, ±4%) so repeats don't sound identical. Mute/volume persisted in `localStorage` (`rumrunner.soundMuted`/`rumrunner.soundVolume`); `subscribeMuted` for `useSyncExternalStore`. Public API unchanged from the HTMLAudioElement era; volume API still has no UI. |
| `src/audio/holdPolicy.ts` | `createHoldGate(schedule)` — pure one-sound-per-moment gate (branding.md §4). `cueFamily(id)` is an exhaustive switch (no default) partitioning every `SoundId` into `action` (draw-stock/draw-discard/pile-dive/meld/layoff/discard), `outcome` (hand-over/go-out/gin/undercut/game-over/hand-cancelled), or `other`. `other` plays immediately; `action` cues hold for `ACTION_HOLD_MS = 80` ms unless an `outcome` cue arrives first, which cancels every held action (dropped silently) and plays itself immediately. Scheduler is injected (real `setTimeout`/`clearTimeout` from `sounds.ts`, a fake one in tests) so the gate itself is synchronous and unit-tested. |
| `src/components/SoundToggle.tsx` | Mute icon button (🔊/🔇) via `useSyncExternalStore(subscribeMuted, getMuted)`; constant `aria-label` + `aria-pressed` (ARIA toggle pattern). Mounted in Room header + Lobby. |
| `src/assets/sounds/` | 21 wav assets (mono 32 kHz PCM, ~800 kB), imported as URLs in `sounds.ts`. Sourced from Kenney CC0 packs (originals + licensing: `assets/sounds-src/PROVENANCE.md`); regenerate with `node packages/client/scripts/process-sounds.mjs` (mapping/params in `scripts/sound-manifest.mjs`, decodes via the `ffmpeg-static` dev dep). `scripts/generate-sounds.mjs` (zero-dep synth, seeded PRNG) is the fallback generator for any id lacking a good sourced sample. |
| `src/routes/Home.tsx` | Create/join forms. Shows error banner for pre-join errors. |
| `src/routes/Room.tsx` | View switch: renders `Lobby` (while `publicState === null`) or the game layout (header row, error banner, `Table`+`MeldZone`+`Chat`, `ActionBar`+`Hand`). Mounts `ConnectionBanner`, `DisconnectWarningModal`, `ScoreOverlay`, `HowToPlayModal` in both views as applicable. |
| `src/routes/Lobby.tsx` | Lobby waiting room card — player list, host `HouseRuleConfig` / non-host `HouseRuleSummary`, Start Game (host) / waiting text, How to Play, `LeaveButton`. |
| `src/components/ScoreOverlay.tsx` | Hand-end modal with per-player card breakdown (melded credits + unmelded cards with point badges), gin result lines, hand-cancelled banner, Re-deal/New Hand/Play Again host button. Owns the house-rule-aware card-point helpers (`cardPtsBasic`/`cardPts500`/`sortCardsDesc`). |
| `src/components/ConnectionBanner.tsx` | Fixed banner (both views): `reconnecting` (amber) / `disconnected` + Reload button (red) from own `connStatus`, or "{opponent} disconnected — waiting…" (amber) from `opponentConn`. |
| `src/components/ConfirmModal.tsx` | Styled yes/no confirmation modal on the `Modal` primitive. Consumed by `LeaveButton`, `DisconnectWarningModal`. |
| `src/components/LeaveButton.tsx` | Leave-game button + `ConfirmModal` step; leaving cancels the game for everyone. |
| `src/components/DisconnectWarningModal.tsx` | Prompt when `disconnectWarning` fires (player silent >5min) — cancel game or keep waiting. |
| `src/components/OpponentStrip.tsx` | Compact per-player info strip above the table (name, hand count, score, turn marker, forfeited state). |
...
| `src/components/HouseRuleConfig.tsx` | NS-8 host editor — renders only `supportedDefs(variant)`; Home create collapsed disclosure + Lobby host panel; amber 'house rule' chip on non-canonical rows; 'Reset to canonical'; empty-registry fallback text. |
| `src/components/HouseRuleSummary.tsx` | NS-8 disclosure — `deviations()` diff vs registry canonical, default list export + `HouseRuleChip` (header chip → Modal popover). Mounts: Lobby (non-host), game header, HowToPlayModal 'Table house rules'. |
...
| `src/content/howToPlay/basic.tsx` | Static Basic Rummy rules fragment — objective, turn flow, melds, scoring, configurable house rules (canonical defaults + "Table house rules" deviation note, NS-8). |
| `src/content/howToPlay/rum500.tsx` | Static 500 Rummy rules fragment — pile dive, ace-either-end, multi-meld turns, layoff credit. |
| `src/content/howToPlay/gin.tsx` | Static Gin Rummy rules fragment — upcard offer, knock/gin/undercut scoring, layoff after knock, stock-depletion cancel, canonical-only (no house rules for this game variation). |

`packages/client/vitest.config.ts` mirrors the server's: aliases `@online-rummy/shared` → `../shared/src/index.ts`, node environment. Client tests live next to their module (`src/**/*.test.ts`).
...
- **Going Rummy detection** (rules.md A.1.7) uses `state.meldedBy` — if no entry maps to the winner's id, the winner placed no card all hand → score × 2. The previous `hasMeldedEver` flag has been removed (Phase 0 refactor). The "layoff-requires-prior-meld" house rule (rules.md A.1.6 step 3) is implemented — applyLayoff throws ERR_LAYOFF_REQUIRES_MELD when enabled.
- Going-rummy bonus = doubled per-opponent contribution (basic rules.md A.1.7), implemented as score×2 on the winner's hand-end credit. Under the goingRummyFlat10 house rule (rules.md A.1.7) the winner instead gets earned + 10.
...
- **NS-8 house rules.** Config flows create/`setHouseRules` → `validateHouseRules` (unknown id/type ⇒ `ERR_INVALID_HOUSE_RULE`; enabling a `supported:false` def ⇒ `ERR_UNSUPPORTED_HOUSE_RULE`) → `Room.houseRules` (canonical-merged) → `GameState.houseRules` at createGame → `PublicState.houseRules` on the wire. Engine tests a flag as `state.houseRules.<id> === true`. UI renders editors from `supportedDefs` only ([E9]) and derives every deviation display by diffing against registry canonicals ([S9] — never hardcode a canonical).
