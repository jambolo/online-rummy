...
| `src/components/HouseRuleSummary.tsx` | NS-8 disclosure — `deviations()` diff vs registry canonical, `SCORING_HOUSE_RULE_IDS`, default list export + `HouseRuleChip` (header chip → Modal popover). Mounts: Lobby (non-host), game header, HowToPlayModal 'Table house rules', ScoreOverlay scoring line. |
