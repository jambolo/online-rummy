...
export default function ActionBar() {
  ...
  const knockerWentGin = isGin && ginKnockerId !== null && publicState.players.find((p) => p.id === ginKnockerId)?.handCount === 0;
  ...
  const deadwoodCards = hand.filter((c) => !knockMeldedIds.has(c.id) && c.id !== knockDiscardId);
  ...
  return (
    ...
    >
      ...
      {isGin && isMyTurn && phase === 'firstUpcardOffer' && publicState.discardTop && (
        <>
          <button className="primary" onClick={() => send({ t: 'draw', from: 'discard' })}>
            Take {publicState.discardTop.rank}
            {SUIT_SYMBOL[publicState.discardTop.suit]}
          </button>
          <button onClick={() => send({ t: 'passUpcard' })}>Pass</button>
          <span style={{ fontSize: 12, color: t.text55 }}>Opening upcard offer — take it or pass to opponent.</span>
        </>
      )}
      ...
      {isMyTurn && phase === 'draw' && (
        <>
          <button className="primary" onClick={() => send({ t: 'draw', from: 'stock' })} disabled={publicState.stockSize === 0}>
      ...
      {isGin && isMyTurn && phase === 'discard' && (
        <>
              ...
              {knockMelds.map((group, i) => {
                ...
                return (
                  <span key={i} style={chipMeld}>
                    ...
                    <button onClick={() => removeKnockMeld(i)} style={chipRemoveBtn}>
                      ×
                    </button>
                  </span>
                );
          ...
          >
            Deadwood: {deadwoodValue}
            {deadwoodValue <= 10 ? ' ✓' : ` (need ≤10)`}
          </span>
          ...
          {canKnock && (
            <button className="primary" onClick={doKnock} style={{ background: deadwoodValue === 0 ? t.accentGin : undefined }}>
        ...
        <>
          {isMyTurn &&
            (() => {
              ...
              const defSelCards = sel.map((id) => hand.find((c) => c.id === id)).filter((c): c is Card => c !== undefined);
              const defSelAlreadyMelded = sel.some((id) => ginDefenderMeldedIds.has(id));
              const canAddDefenderMeld = sel.length >= 3 && !defSelAlreadyMelded && isValidGinMeld(defSelCards);
                ...
                totalActions === 0
                  ...
                  : `Submit ${[
                      ginDefenderMelds.length > 0
                        ...
                        : null,
                      ginLayoffs.length > 0 ? `${ginLayoffs.length} layoff${ginLayoffs.length > 1 ? 's' : ''}` : null,
                    ]
                      .filter(Boolean)
                      .join(' + ')}`;
                    ...
                    <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
                      {ginDefenderMelds.map((group, i) => {
                        ...
                        return (
                          <span key={i} style={chipMeld}>
                            {groupCards.map(cardLabel).join(' ')}
                            <button onClick={() => removeGinDefenderMeld(i)} style={chipRemoveBtn}>
                              ×
                            </button>
                          </span>
                        );
                      ...
                      {ginLayoffs.map((layoff, i) => {
                        ...
                        return (
                          <span key={i} style={chipLayoff}>
                            ...
                            <button onClick={() => removeGinLayoff(i)} style={chipRemoveBtn}>
                              ×
                            </button>
                          </span>
                        );
          ...
          {!isMyTurn && (
            <span style={{ fontSize: 12, color: t.text55 }}>
              {knockerWentGin ? 'Opponent is arranging their melds…' : 'Opponent is declaring melds and laying off…'}
            </span>
          )}
        </>
      )}
      ...
      {sel.length > 0 && (
        <button onClick={clearSelect} style={{ marginLeft: 'auto', opacity: 0.7 }}>
          ...
          return (
            ...
            >
              <div style={{ fontSize: 14, marginBottom: 16 }}>
                {card ? `${card.rank}${SUIT_SYMBOL[card.suit]}` : 'This card'} can be laid off on a meld. Discard it anyway?
              </div>
