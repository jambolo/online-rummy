...
- **Action events** are emitted **before** each action's state broadcast, so clients can key sounds and animations off events without diffing state. `draw`→`drew` carries `DrewEventData` with `from` normalized server-side, never echoed raw. Action events also name the cards they moved, under one invariant: **only cards already public** (face-up in the discard pile, or placed on the table). So `drew {from:'stock'}` carries no `cards` and `knocked` names the declared melds but never the face-down knock discard (rules.md A.2.4). Ids resolve via `cardRegistry` (stable for the whole game); any draw off the discard pile needs the pre-action `discardPile` snapshot `ws.ts` takes, because the draw removes what it names.
