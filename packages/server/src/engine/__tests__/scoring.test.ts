...
describe('deadwood', () => {
  ...
  it('empty hand → 0', () => {
    const player = {
      id: 'p', name: 'P', melds: [], score: 0, status: 'active' as const, hand: [],
    };
...
describe('validateKnockMelds', () => {
  ...
  it('valid set + run → both validated, meldedIds union populated', () => {
    const hand = [
      c('5', 'C', 's1'), c('5', 'D', 's2'), c('5', 'H', 's3'),
      c('A', 'C', 'r1'), c('2', 'C', 'r2'), c('3', 'C', 'r3'),
    ];
    const { state, player } = setup(hand);
    const { validated, meldedIds } = validateKnockMelds(
      state, player, [['s1', 's2', 's3'], ['r1', 'r2', 'r3']], ginValidate,
    );
  ...
  it('card in two groups → ERR_CARD_IN_MULTIPLE_MELDS', () => {
    const hand = [
      c('7', 'C', 'a'), c('7', 'D', 'b'), c('7', 'H', 'c'), c('7', 'S', 'd'),
      c('8', 'C', 'e'), c('8', 'D', 'f'),
    ];
    const { state, player } = setup(hand);
    expect(() =>
      validateKnockMelds(state, player, [['a', 'b', 'c'], ['c', 'd', 'e']], ginValidate),
  ...
  it('card not in hand → ERR_CARD_NOT_IN_HAND', () => {
    ...
    const { state, player } = setup(hand);
    expect(() =>
      validateKnockMelds(state, player, [['a', 'b', 'missing']], ginValidate),
    ).toThrow('ERR_CARD_NOT_IN_HAND:missing');
  });
  ...
  it('group fails validate → ERR_INVALID_MELD', () => {
    ...
    const { state, player } = setup(hand);
    expect(() =>
      validateKnockMelds(state, player, [['x', 'y', 'z']], ginValidate),
    ).toThrow('ERR_INVALID_MELD');
  });
});
